...
int main(void) {
    int i = 0;
    char nom[SIZE], nouvelle_valeur[SIZE];
    char env_entry[2 * SIZE];  // Array to store environment entry in the form "name=value"
    char *variable;
    ...
    while (1) {
        ...
        fprintf(stdout, "entrez la nouvelle valeur de la variable d'environnement : ");
        scanf("%s", nouvelle_valeur);

        // Form the environment entry "name=value"
        snprintf(env_entry, sizeof(env_entry), "%s=%s", nom, nouvelle_valeur);

        // Modifier la variable d'environnement
        if (_putenv(env_entry) != 0) {
            perror("Erreur lors de la modification de la variable d'environnement");
        }
